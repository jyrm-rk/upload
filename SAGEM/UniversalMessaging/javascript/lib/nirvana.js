...
/**Create temp object to be implemented into nirvana_core file */
var nirvanaObjTemp = {};

(function() {
	var nirvanaJS = {};
	var nirvana_core_temp = {};

	var driverDomObjCounter = 0;
	/***Return driver dom Object container obj  */
	function driverDomObjectsContainerID() {
		if (driverDomObjCounter === 0) {
			driverDomObjCounter++;
			return null;
		} else {
			return DriverDomObjectsContainerID;
		}
	}
	...
	function showDebugConsole() {
		if (
			!nirvana_core_temp.log.debugConsoleOpened ||
			(nirvana_core_temp.log._console && nirvana_core_temp.log._console.closed)
		) {
			nirvana_core_temp.log._console = window.open(
				'',
				'Console',
				'width=600,height=300,scrollbars=yes,resizable'
			);
			nirvana_core_temp.log._console.document.open();
			nirvana_core_temp.log._console.document.writeln(
				'<style> { font-size: 10px; font-family: verdana, lucida sans, courier new, courier, sans-serif ; }</style>'
			);
			nirvana_core_temp.log._console.document.writeln('<b>User Agent: ' + navigator.userAgent + '</b><br/>');
			for (var i = 0; i < nirvana_core_temp.log.logs.length; i++) {
				nirvana_core_temp.log._console.document.writeln(nirvana_core_temp.log.logs[i] + '<br/>');
			}

			nirvana_core_temp.log.debugConsoleOpened = true;
		} else {
			nirvana_core_temp.log._console.focus();
		}
	}

	/***HTTP request Factory - requestBuilder
	 *get string  navigator application name  */
	function navigatorAppname() {
		return navigator.appName.replace(' ', '-');
	}

	/***HTTP request Factory - requestBuilder
	 *get string  navigator platform name  */
	function navigatorPlatform() {
		return window.navigator.platform.toString().replace(' ', '_');
	}

	function initXMLHTTP(name) {
		// If IE7, Mozilla, Safari, etc: Use native object
		if (nirvana_core_temp.Utils.isLoggingEnabled()) {
			nirvana_core_temp.Utils.Logger.log(2, '[initXMLHTTP] for ' + name);
		}
		...
		if (typeof window.XMLHttpRequest != 'undefined' && window.XMLHttpRequest) {
			try {
				...
				if (nirvana_core_temp.Utils.isLoggingEnabled()) {
					nirvana_core_temp.Utils.Logger.log(
						2,
						'initXMLHTTP : Constructed XMLHttpRequest for [' + name + ']'
					);
				}
				return thisXMLHttp;
			} catch (anex) {
				if (nirvana_core_temp.Utils.isLoggingEnabled()) {
					nirvana_core_temp.Utils.Logger.log(
						2,
						'initXMLHTTP : Error constructing XMLHttpRequest for [' + name + ']. error=' + anex.message
					);
				}
				thisXMLHttp = false;
			}
			...
			for (var a = 0; a < vers.length; a++) {
				try {
					...
					if (nirvana_core_temp.Utils.isLoggingEnabled()) {
						nirvana_core_temp.Utils.Logger.log(
							2,
							'initXMLHTTP : Constructed ActiveXObject XMLHttpRequest using [' +
								vers[a] +
								'] for [' +
								name +
								']'
					...
					if (nirvana_core_temp.Utils.isLoggingEnabled()) {
						nirvana_core_temp.Utils.Logger.log(
							2,
							'initXMLHTTP : Error constructing ActiveXObject XMLHttpRequest :' +
								vers[a] +
								' for [' +
								name +
								']. error=' +
			...
			if (nirvana_core_temp.Utils.isLoggingEnabled()) {
				nirvana_core_temp.Utils.Logger.log(
					2,
					'initXMLHTTP : Failed to construct xmlHTTP object for [' + name + ']'
				);
			}
			thisXMLHttp = false;
		}
	...
	function setPreferredDrivers(
		userDriverArray,
		applyUserAgentHeuristicsFlag,
		usePreviousBestDriverFlag,
		currentSession
	) {
		if (usePreviousBestDriverFlag && nirvana_core_temp.preferredDrivers.length) {
			return;
		}
		if (typeof userDriverArray == 'undefined') {
			nirvana_core_temp.preferredDrivers = defaultDrivers;
		} else {
			var sanitizedDriverArray = [];
			for (var i = 0; i < userDriverArray.length; i++) {
				var userDriverName = userDriverArray[i];
				if (typeof window.Nirvana.Driver[userDriverName] != 'undefined') {
					sanitizedDriverArray.push(userDriverName);
				} else {
					if (typeof window === 'undefined') {
						console.log('Node js Implementation');
						sanitizedDriverArray.push('WEBSOCKET');
					} else {
						nirvana_core_temp.ListenerManagers[window.Nirvana].notifyListeners(
							window.Nirvana.Observe.ERROR,
							currentSession,
							new nirvana_core_temp.exceptions.InvalidDriverException(userDriverName)
						);
					}
				}
			}
			if (sanitizedDriverArray.length) {
				nirvana_core_temp.preferredDrivers = sanitizedDriverArray;
			} else {
				nirvana_core_temp.preferredDrivers = defaultDrivers;
			}
		}
		if (applyUserAgentHeuristicsFlag) {
			nirvana_core_temp.preferredDrivers = applyUserAgentHeuristics(nirvana_core_temp.preferredDrivers);
		}

		var currentDriverIndex = 0;

		// If the developer wants the client to always start with the "best" driver it used before, then do so:
		if (nirvana_core_temp.Transport.getConfig().rememberDriver) {
			var rememberedDriver = nirvana_core_temp.Utils.getCookie('NirvanaDriver');
			if (rememberedDriver !== null) {
				for (var x = 0; x < nirvana_core_temp.preferredDrivers.length; x++) {
					if (nirvana_core_temp.preferredDrivers[x] == rememberedDriver) {
		...
		nirvana_notifyListeners.notifyListeners(
			window.Nirvana.Observe.DRIVER_CHANGE,
			currentSession,
			nirvana_core_temp.preferredDrivers[currentDriverIndex]
		);
	}
	...
	function applyUserAgentHeuristics(driverArray) {
		...
		if (ieVersion() == 9) {
			disableDriver(nirvanaObj.Drivers.Names.JSONP_LONGPOLL);
		}
	...
	function initialize() {
		...
		var createDriverDomObjectsContainer = function() {
			try {
				if (document.getElementById(nirvanaJS.driverDomObjectsContainerID()) === null) {
	...
	*/
	...
	function CrossDomainProxy(session, driverName, proxyRealm, initCallback, errorCB) {
		...
		this.proxyIFrame = createIFrame(
			this.proxyURL +
				'?proxyID=' +
				this.proxyID +
				'&sessionID=' +
				nirvana_core_temp.Utils.generateUniqueID() +
				'&validOrigins=' +
				encodeURIComponent(session.getConfig().currentDomain + ',' + session.getConfig().realms.join(',')),
			errorCB
		);
		this.readyState = 0;
		this.responseText = '';
		this.status = 0;
		nirvana_core_temp.postMessageProxies.collection[this.proxyID] = this;
	}
	...
	CrossDomainProxy.prototype.destroy = function() {
		...
		delete nirvana_core_temp.postMessageProxies.collection[this.proxyID];
		nirvana_core_temp.postMessageProxies.collection[this.proxyID] = null;
	...
	function createForeverIFrame(url, onMessageCB, onErrorCB) {
		...
		if (document.createElement) {
			...
			if (element) {
				...
				if (nirvana_core_temp.Utils.isLoggingEnabled()) {
					nirvana_core_temp.Utils.Logger.log(2, 'Iframe does not exist, so creating new one');
				}
    ...
    */
	...
	function JSONP_Poller(errorCB) {
		this.id = nirvana_core_temp.Utils.generateUniqueID();

		if (nirvana_core_temp.Utils.isLoggingEnabled()) {
			nirvana_core_temp.Utils.Logger.log(2, '[JSONP_Poller] Creating new longpoller: ' + this.id);
		}
		...
		this.send = function(url, data) {
			...
			if (nirvana_core_temp.Utils.isLoggingEnabled()) {
				nirvana_core_temp.Utils.Logger.log(2, '[JSONP_Poller] Sending ' + scriptSrc);
			}
			try {
				if (hasInitialised) {
    ...
    */
		...
		function(e) {
			...
			if (!isCrossDomainProxyMessage(e.data)) {
				if (nirvana_core_temp.Utils.isLoggingEnabled()) {
					nirvanaJS.log(
						0,
						'Window received message event that is not intended as cross domain proxy communication.'
					);
				}
				return;
			}
			// Do we trust the sender of this message?
			if (!isTrustedPostMessageSender(nirvana_core_temp.Utils.getCanonicalOrigin(e.origin))) {
				nirvana_core_temp.ListenerManagers[window.Nirvana].notifyListeners(
					window.Nirvana.Observe.ERROR,
					null, // since this message is not associated with any session.
					new nirvana_core_temp.exceptions.DriverSecurityException(
						'CrossDomainProxy',
						'PostMessage Event Origin ' +
							e.origin +
							' does not equal any proxied realm origin ' +
							getPostMessageProxyRealms()
					)
				);
				return;
			}
			...
			var XHR = nirvana_core_temp.postMessageProxies.collection[proxyAlias];
			if (action == 'init' && XHR) {
			...
			if (action == 'iframe') {
				nirvana_core_temp.Transport.c(JSON.parse(dataArray[3]));
			}

			if (action == 'iFrame_close') {
				nirvana_core_temp.Transport.onError(action);
			}
	...
	function openListenerManager(currentSession, nvde) {
		return nirvana_core_temp.ListenerManagers[window.Nirvana].notifyListeners(
			window.Nirvana.Observe.ERROR,
			currentSession,
			nvde
		);
	}
	function notifyListenersManagerStart(client, sessionEx) {
		return nirvana_core_temp.ListenerManagers[window.Nirvana].notifyListeners(
			window.Nirvana.Observe.ERROR,
			client,
			sessionEx
	...
	function getElementofDriverDomOnjContainer() {
		return window.document.getElementById(nirvanaJS.driverDomObjectsContainerID());
	}

	//subscribe
	function subscribeListenerManager(client, ase) {
		nirvana_core_temp = nirvana_core;
		return nirvana_core_temp.ListenerManagers[window.Nirvana].notifyListeners(
			window.Nirvana.Observe.ERROR,
			client,
			ase
	...
	function addClientEventListener(bu) {
		if (window.addEventListener) {
	...
	function subscribeNotifyListener(client, ex) {
		return nirvana_core_temp.ListenerManagers[window.Nirvana].notifyListeners(
			window.Nirvana.Observe.ERROR,
			client,
			ex
	...
	function getFormatedUrlRealm(config) {
		...
		if (document.location.host.match(/:[0-9]+$/)) {
			...
			realm = nirvana_core_temp.Utils.getCanonicalOrigin(realm);
		}
		config.realms = [ realm ];
	...
	function getCurrentDomainUrl() {
		...
		if (document.location.protocol != 'file:') {
			if (document.location.host.match(/:[0-9]+$/)) {
				...
				currentDomain = nirvana_core_temp.Utils.getCanonicalOrigin(currentDomain);
			}
		}
	...
	function isCrossDomainProxyMessage(messageData) {
		...
		if (messageComponents.length > 0) {
			...
			if (
				tmp.indexOf('EVENTSOURCE_STREAMING_POSTMESSAGE') != -1 ||
				tmp.indexOf('XHR_LONGPOLL_POSTMESSAGE') != -1 ||
	...
	function isTrustedPostMessageSender(origin) {
		...
		for (var proxyAlias in nirvana_core_temp.postMessageProxies.collection) {
			if (nirvana_core_temp.postMessageProxies.collection.hasOwnProperty(proxyAlias))
				if (nirvana_core_temp.postMessageProxies.collection[proxyAlias])
					if (
						nirvana_core_temp.Utils
							.getCanonicalOrigin(nirvana_core_temp.postMessageProxies.collection[proxyAlias].proxyRealm)
							.toUpperCase() == origin
					)
						return true;
		}
	...
	function log(severity, msg) {
		if (nirvana_core_temp.log.debugLevel <= severity) {
			var msgArgs = Array.prototype.slice.call(arguments);
			msgArgs.unshift(new Date());
			var logMessage = msgArgs.join(' ');
			if (
				nirvana_core_temp.log.debugConsoleOpened &&
				nirvana_core_temp.log._console &&
				!nirvana_core_temp.log._console.closed
			)
				nirvana_core_temp.log._console.document.writeln(logMessage + '<br/>');
			if (nirvana_core_temp.log.logs.length > nirvana_core_temp.log.maxLogLength) {
				nirvana_core_temp.log.logs.shift();
			}
			nirvana_core_temp.log.logs.push(logMessage);
			nirvana_core_temp.log.logFunction(logMessage);
		}
	}
	...
	function implNirvanaCore(nirvana_core) {
		nirvana_core_temp = nirvana_core;
	}
	/*
    *  ************************************************************************************
    ...
    */
	...
	function FormSubmitter() {
		...
		if (nirvana_core_temp.Utils.isLoggingEnabled()) {
			nirvana_core_temp.Utils.Logger.log(2, '[FormSubmitter] Creating new FormSubmitter: ' + this.id);
		}

		this.send = function(url, data) {
			if (nirvana_core_temp.Utils.isLoggingEnabled()) {
				nirvana_core_temp.Utils.Logger.log(2, '[FormSubmitter] Sending ' + url + '?' + data[0] + '&' + data[1]);
			}
			cleanNode(scriptContainer, isIE);
			...
			try {
				...
				if (nirvana_core_temp.Utils.isLoggingEnabled()) {
					nirvana_core_temp.Utils.Logger.log(2, '[FormSubmitter] Exception: ' + ex.message);
				}
			}
			form.submit();
		};
	...
	var Drivers = (function() {
		...
		DriverImplementations[Names.WEBSOCKET] = function(driverName) {
			...
			this.open = function() {
				...
				if (!this.isClientSupported()) {
					nirvana_core_temp.Transport.onError(this.name + ' driver not supported by client');
					return;
				}
				...
				var wsurl;
				var loc = window.document.createElement('a'); // try Utils.getUrlProtocol(Transport.getCurrentRealm())
				loc.href = nirvana_core_temp.Transport.getCurrentRealm();
				...
				if ('WebSocket' in window) {
					currentSocket = new window.WebSocket(wsurl);
				...
				var onErrorClose = function(parameter) {
					if (isClosed) {
						nirvana_core_temp.Transport.onClose();
					} else {
			...
			this.send = function(command, data) {
				...
				if (nirvana_core_temp.Transport.isConnected()) {
					nirvana_core_temp.OutboundEngine.setReady(true);
				}
			};
		...
		DriverImplementations[Names.XHR_STREAMING_CORS] = function(driverName) {
			...
			var isClosed = false;
			var maxStreamLength = nirvana_core_temp.Transport.getCurrentSession().getConfig()
				.maxStreamLengthStreamingCors;
			...
			this.open = function() {
				...
				rname = nirvana_core_temp.Transport.getCurrentRealm() + '/sv/nvLite';
					...
					if (nirvana_core_temp.Utils.isLoggingEnabled()) {
						nirvana_core_temp.Utils.Logger.log(
							...
							'[' +
								...
								' Receiver] ' +
								this.readyState +
								' : CumulativeResponseLength=' +
								this.responseText.length
						);
					}
					if (this.readyState == 3) {
						currentChunkPosition = DriverUtils.processResponseChunk(
							this.responseText,
						...
						if (this.responseText.length >= maxStreamLength && !isClosingDown) {
							...
							if (nirvana_core_temp.Utils.isLoggingEnabled()) {
								nirvana_core_temp.Utils.Logger.log(
									...
									'[' +
										driverName +
										' Receiver] Reached max stream length ' +
										maxStreamLength +
										', resetting.'
								);
							}
							nirvana_core_temp.OutboundEngine.prioritizeCommand({
								requestType: nirvana_core_temp.PrivateConstants.CLIENT_CLOSE,
								clientRequest: false
							});
						}
					} else if (this.readyState == 4 && !isClosingDown) {
						nirvana_core_temp.Transport.onError('Receiver Connection Closed = ' + this.status);
					}
				};
				...
				sender.onreadystatechange = function() {
					...
					if (this.readyState == 4 && this.status != 200) {
						nirvana_core_temp.Transport.onError('Sender Connection Status = ' + this.status);
					} else if (this.readyState > 3) {
						nirvana_core_temp.Transport.c(this.responseText);
						nirvana_core_temp.OutboundEngine.setReady(true);
					}
				};
				nirvana_core_temp.Transport.onOpen();
			};

			this.send = function(command, data) {
				if (!isClosed) {
					if (command.requestType === nirvana_core_temp.PrivateConstants.SESSION_START) {
		...
		DriverImplementations[Names.IFRAME_STREAMING_POSTMESSAGE] = function(driverName) {
			...
			this.isClientSupported = function() {
				...
				return (
					(typeof window.postMessage === 'function' || typeof window.postMessage === 'object') && !konqueror
				);
			};
			...
			this.open = function() {
				...
				rname = nirvana_core_temp.Transport.getCurrentRealm() + '/sv/nvLite';
				nirvana_core_temp.OutboundEngine.setReady(false);
				receiver = sender = new CrossDomainProxy(
					nirvana_core_temp.Transport.getCurrentSession(),
					this.name,
					nirvana_core_temp.Transport.getCurrentRealm(),
					opened,
					nirvana_core_temp.Transport.onError
				);
				sender.onreadystatechange = function() {
					...
					if (nirvana_core_temp.Utils.isLoggingEnabled()) {
						nirvana_core_temp.Utils.Logger.log(
							1,
							'[' + driverName + ' Sender] ' + this.readyState + ' : ' + this.responseText
						);
					}
					if (this.readyState == 4) {
						if (this.status != 200) {
							nirvana_core_temp.Transport.onError(driverName + ' sender status ' + this.status);
						} else {
							if (isOpened) {
								nirvana_core_temp.OutboundEngine.setReady(true);
							}
			...
			function connectStreamingReceiver(data) {
				...
				receiver.proxyRequest('createForeverIFrame', [
					nirvana_core_temp.Transport.getCurrentRealm() + '/sv/nvLite' + '?' + data
				]);
			}
		...
		DriverImplementations[Names.EVENTSOURCE_STREAMING_POSTMESSAGE] = function(driverName) {
			...
			function sendCommand(command, data) {
				...
				sender.open('POST', nirvana_core_temp.Transport.getCurrentRealm() + '/sv/nvLite' + '?' + data[0], true);
				sender.send(data[1]);
			}
		...
		DriverImplementations[Names.XHR_LONGPOLL_CORS] = function(driverName) {
			...
			this.open = function() {
				...
				receiver.onreadystatechange = function() {
					if (nirvana_core_temp.Utils.isLoggingEnabled()) {
						nirvana_core_temp.Utils.Logger.log(
							1,
							'[' + driverName + '] receiver (' + this.readyState + ') : ' + this.responseText
						);
					}
					if (this.readyState == 4) {
						if (this.status != 200) {
							nirvana_core_temp.Transport.onError(driverName + ' receiver status ' + this.status);
						} else {
							nirvana_core_temp.Transport.d(this.responseText);
							if (!isClosed) {
			...
			this.send = function(command, data) {
				if (!isClosed) {
					if (
						command.requestType === nirvana_core_temp.PrivateConstants.KEEP_ALIVE ||
						command.requestType === nirvana_core_temp.PrivateConstants.SESSION_START
					) {
				...
				if (!isClosed) {
					...
					try {
						...
						if (nirvana_core_temp.Utils.isLoggingEnabled()) {
							nirvana_core_temp.Utils.Logger.log(
								1,
								'[' + driverName + '] exception on close(): ' + ex.message
							);
						}
					}
					receiver = null;
		...
		DriverImplementations[Names.XHR_LONGPOLL_POSTMESSAGE] = function(driverName) {
			...
			this.open = function() {
				...
				receiver = new CrossDomainProxy(
					nirvana_core_temp.Transport.getCurrentSession(),
					driverName,
					nirvana_core_temp.Transport.getCurrentRealm(),
					notifyTransportOpenWhenReady,
					nirvana_core_temp.Transport.onError
				);
				receiver.onreadystatechange = function() {
					if (nirvana_core_temp.Utils.isLoggingEnabled()) {
			...
			this.send = function(command, data) {
				if (!isClosed) {
					if (command.requestType === nirvana_core_temp.PrivateConstants.KEEP_ALIVE) {
						if (nirvana_core_temp.Utils.isLoggingEnabled()) {
							nirvana_core_temp.Utils.Logger.log(1, '[' + driverName + '] receiver requesting: ' + data);
						}
						receiver.open('GET', rname + '?' + data[0]);
						receiver.send();
					} else {
						if (nirvana_core_temp.Utils.isLoggingEnabled()) {
							nirvana_core_temp.Utils.Logger.log(1, '[' + driverName + '] sender requesting: ' + data);
						}
						sender.open('POST', rname + '?' + data[0]);
						sender.send(data[1]);
					}
		...
		DriverImplementations[Names.JSONP_LONGPOLL] = function(driverName) {
			...
			function receiverResponseHandler(data) {
				if (nirvana_core_temp.Utils.isLoggingEnabled()) {
					nirvana_core_temp.Utils.Logger.log(1, '[' + driverName + '] receiverResponseHandler: ' + data);
				}
				if (nirvana_core_temp.Transport.c(data)) {
					if (nirvana_core_temp.Transport.isConnected()) {
						nirvana_core_temp.OutboundEngine.setReady(true);
					}
				}
			...
			this.open = function() {
				...
				receiver = new nirvana_core_temp.Utils.JSONP_Poller(nirvana_core_temp.Transport.onError);
				sender = new FormSubmitter();
				nirvana_core_temp.Transport.onOpen();
			};
			...
			this.send = function(command, data) {
				if (!isClosed) {
					if (
						...
						if (nirvana_core_temp.Utils.isLoggingEnabled()) {
							nirvana_core_temp.Utils.Logger.log(1, '[' + driverName + '] Receiver Req: ' + data[0]);
						}
						...
						if (nirvana_core_temp.Utils.isLoggingEnabled()) {
							nirvana_core_temp.Utils.Logger.log(
								1,
								'[' + driverName + '] Sender Req: ' + data[0] + '&' + data[1]
							);
						}
		...
		DriverImplementations[Names.NOXD_IFRAME_STREAMING] = function(driverName) {
			var currentSession = nirvana_core_temp.Transport.getCurrentSession();
			this.name = driverName;
			var self = this;
			var receiver,
				sender,
			...
			this.isClientSupported = function() {
				...
				for (var i = 0; i < realmLength; i++) {
					var realm = nirvana_core_temp.Utils.getCanonicalOrigin(realms[i]);
					if (rexp.test(realm)) {
						return true;
					}
				}
			...
			function receiverCB(data) {
				if (nirvana_core_temp.Utils.isLoggingEnabled()) {
					nirvana_core_temp.Utils.Logger.log(
						1,
						'[' + driverName + '] receiverCB: ' + typeof data + ' ' + data
					);
				}
				if (data) {
					nirvana_core_temp.Transport.c(data);
				}
			}

			function senderCB(data) {
				if (nirvana_core_temp.Utils.isLoggingEnabled()) {
					nirvana_core_temp.Utils.Logger.log(1, '[' + driverName + '] senderCB: ' + typeof data + ' ' + data);
				}
				if (data) {
					nirvana_core_temp.Transport.d(data);
				}
				nirvana_core_temp.OutboundEngine.setReady(true);
			}

			function errorCB(error, htmlfileWasUsed) {
				nirvana_core_temp.Transport.onError(error);
			}

			function connectStreamingReceiver(data) {
				receiver = createForeverIFrame(rname + '?' + data, receiverCB, errorCB);
				if (nirvana_core_temp.Utils.isLoggingEnabled()) {
					nirvana_core_temp.Utils.Logger.log(1, '[' + driverName + ' Receiver] ' + rname + '?' + data);
				}
			...
			function sendRequest(command, data) {
				if (!isClosed) {
					if (nirvana_core_temp.Utils.isLoggingEnabled()) {
						nirvana_core_temp.Utils.Logger.log(1, '[' + driverName + ' Sender] ' + rname + '?' + data);
					}
					var asyncFlag = command.requestType === nirvana_core_temp.PrivateConstants.CLIENT_CLOSE;
					sender.open('POST', rname + '?' + data[0], asyncFlag);
					sender.onreadystatechange = readyStateCB;
					sender.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
					try {
						if (nirvana_core_temp.Utils.isLoggingEnabled()) {
							nirvana_core_temp.Utils.Logger.log(1, '[' + driverName + '] XHR POST: ' + data);
						}
						sender.send(data[1]);
						return true;
					} catch (ex) {
			...
			function readyStateCB() {
				if (sender === null) {
					if (nirvana_core_temp.Utils.isLoggingEnabled()) {
						nirvana_core_temp.Utils.Logger.log(
							1,
							'[' + driverName + '] statechangecb: Sender is already null'
						);
					}
					return;
				}
				...
				if (nirvana_core_temp.Utils.isLoggingEnabled()) {
					nirvana_core_temp.Utils.Logger.log(
						1,
						'[' + driverName + '] statechangecb: State now changed to ' + thisStateChange
					);
				}
				...
				try {
					...
					if (nirvana_core_temp.Utils.isLoggingEnabled()) {
						nirvana_core_temp.Utils.Logger.log(
							1,
							'[' + driverName + '] statechangecb : failed to read status code'
						);
					}
				}
				if (statusCode === 200 || statusCode === 0) {
					...
					if (nirvana_core_temp.Utils.isLoggingEnabled()) {
						nirvana_core_temp.Utils.Logger.log(
							1,
							'[' + driverName + '] statechangecb - status of ' + statusCode + ' forcing session reset'
						);
					}
					errorCountXHR++;
					nirvana_core_temp.Transport.onError('Error transports has been closed');
				}
		...
		var DriverUtils = (function() {
			...
			function longpoll() {
				var driver = nirvana_core_temp.Transport.getDriver();
				if (driver === null) {
				...
				var command = {
					requestType: nirvana_core_temp.PrivateConstants.KEEP_ALIVE,
					requestID: nirvana_core_temp.Transport.getRequestCountIncrement()
				};
				var request = nirvana_core_temp.Transport.getCurrentSession().requestFactory.buildRequest(command);
				if (nirvana_core_temp.Utils.isLoggingEnabled()) {
					//nirvana_core_temp.Utils.Logger.log(1, "[" + driver.name + "] longpoll() Sending request number " + command.requestID + " of " + nirvana_core_temp.Transport.getRequestCount() + " for " + describe(command.requestType) + " " + request);
					nirvana_core_temp.Utils.Logger.log(
						1,
						'[' +
							driver.name +
							'] longpoll() Sending request number ' +
							command.requestID +
							' of ' +
							nirvana_core_temp.Transport.getRequestCount() +
							' for ' +
							' ' +
							request
					);
			...
			function scheduleLongpollTimeout(driverName) {
				longpollTimeoutID = setTimeout(function() {
					nirvana_core_temp.Transport.onError(driverName + ' Longpoll Timeout');
				}, nirvana_core_temp.Transport.getConfig().sessionTimeoutMs);
			}
			...
			function processResponseChunk(responseChunk, currentPosition) {
				...
				do {
					...
					if (newPosition === 1) {
						if (
							responseChunk.charAt(responseChunk.length - 1) == ';' &&
							responseChunk.charAt(responseChunk.length - 2) == ']'
						) {
					...
					if (newPosition > currentPosition) {
						nirvana_core_temp.Transport.d(responseChunk.substring(currentPosition, newPosition - 1));
	...
	/**Export nirvana browsers functions and objects into nirvanaJS
	 * this should be used by nirvana_core to run browser logic for
	 * js library
	 */
	nirvanaObjTemp = {
		implNirvanaCore: implNirvanaCore,
		Drivers: Drivers,
		getCookie: getCookie,
		setCookie: setCookie,
		log: log,
		getPostMessageProxyRealms: getPostMessageProxyRealms,
		createIFrame: createIFrame,
		getCurrentDomainUrl: getCurrentDomainUrl,
		getFormatedUrlRealm: getFormatedUrlRealm,
		subscribeNotifyListener: subscribeNotifyListener,
		subscribeListenerManager: subscribeListenerManager,
		addClientEventListener: addClientEventListener,
		getElementofDriverDomOnjContainer: getElementofDriverDomOnjContainer,
		notifyListenersManagerStart: notifyListenersManagerStart,
		openListenerManager: openListenerManager,
	//	createSessionConfig: createSessionConfig,
		JSONP_Poller: JSONP_Poller,
		debuggerKeyListener: debuggerKeyListener,
		navigatorAppname: navigatorAppname,
		navigatorPlatform: navigatorPlatform,
		initXMLHTTP: initXMLHTTP,
		setPreferredDrivers: setPreferredDrivers,
		driverDomObjectsContainerID: driverDomObjectsContainerID,
		initialize: initialize,
		applyUserAgentHeuristics: applyUserAgentHeuristics
	};
	...
	nirvanaJS = nirvanaObjTemp;

})(window);

/**function to implement Nirvana browser functionality into nirvana core
 * 
 */
function implnirvanaObj() {
	implnirvanaObjTemp(nirvanaObjTemp);
	nirvanaObjTemp = {};
}/*!
 *
...
About: Version & Support
    ...
    JavaScript API Version BuildNumberHere.
    Released 12-Feb-1964.
...
/*** */
var isBrowser =
	typeof window !== 'undefined' && typeof global !== 'object' && {}.toString.call(window) === '[object Window]';

(function() {
	var nirvana_notifyListeners = {};
	if (isBrowser) {
		implnirvanaObj();

		var nirvanaObjTempVar = nirvanaObjJS;
		nirvanaObjJS = {};

		window.nirvanaObjJS = {};
		window.nirvana_notifyListeners = nirvana_notifyListeners;
	}
	...
	var Nirvana = (function() {
		...
		nirvanaObj = nirvanaObjTempVar;

		if (!isBrowser) {
			nirvanaObj = require('../node/um-node').nirvanaObj;
		}
		...
		var Utils = (function() {
			...
			var setDebugLevel = function(level) {
				nirvana_core.log.debugLevel = level;
				debugLevel = level;
			...
			var HTTPRequestFactory = function() {
				...
				requestBuilder[PrivateConstants.SESSION_START] = function(command) {
					...
					return [
						'F=' +
							PrivateConstants.SESSION_START +
							'&Connection-Type=JC&Y=10' +
							'&V=' +
							nirvanaObj.navigatorAppname() +
							'&P=' +
							Transport.Drivers.WireProtocol[Transport.getCurrentDriver()] +
							'&W=' +
							Nirvana.BUILD_NUMBER.replace(' ', '-') +
							'&X=' +
							Nirvana.BUILD_DATE.replace(' ', '-') +
							'&Z=' +
							nirvanaObj.navigatorPlatform() +
							'&S=' +
							encodeURIComponent(session.getUsername()) +
							'&A=' +
							encodeURIComponent(session.getConfig().applicationName) +
							'&G=' +
							(session.getConfig().serverLogging ? 'T' : 'F') +
							'&D=' +
							(session.isDataStreamEnabled() ? 'T' : 'F') +
							'&B=' +
							session.getDataStreamID() +
							'&R=' +
							command.requestID +
							'&1Date=' +
							new Date().getTime(),
				...
				requestBuilder[PrivateConstants.BATCH_SUBSCRIBE] = function(command) {
					...
					return [
						'F=' + PrivateConstants.BATCH_SUBSCRIBE + buildCommonParameters(command),
						'E=' + resourceLength + '&N=' + param
					];
				};
				...
				requestBuilder[PrivateConstants.RESOURCE_PUBLISH] = function(command) {
					...
					return [
						'F=' + PrivateConstants.RESOURCE_PUBLISH + buildCommonParameters(command),
						'N=' + resource.getName() + buildEvent(event)
					];
				};
				...
				requestBuilder[PrivateConstants.RESOURCE_UNSUBSCRIBE] = function(command) {
					...
					var resourceType = resource.getResourceType();
					var request = [
						'F=' + PrivateConstants.RESOURCE_UNSUBSCRIBE + buildCommonParameters(command),
						'N=' + resource.getName()
					];

					if (
						resourceType === Nirvana.QUEUE_RESOURCE ||
						resourceType === Nirvana.TRANSACTIONAL_QUEUE_RESOURCE
					) {
				...
				requestBuilder[PrivateConstants.QUEUE_COMMIT] = function(command) {
					...
					return [
						'F=' + PrivateConstants.QUEUE_COMMIT + buildCommonParameters(command),
						'N=' + resource.getName() + '&E=' + event.getEID()
					];
				};
				...
				requestBuilder[PrivateConstants.QUEUE_ROLLBACK] = function(command) {
					...
					return [
						'F=' + PrivateConstants.QUEUE_ROLLBACK + buildCommonParameters(command),
						'N=' + resource.getName() + '&E=' + event.getEID()
					];
				};
				...
				requestBuilder[PrivateConstants.TX_PUBLISH] = function(command) {
					...
					return [
						'F=' + PrivateConstants.TX_PUBLISH + buildCommonParameters(command),
						'N=' + resource.getName() + '&X=' + transaction.getTxID() + buildEvent(event)
					];
				};
				...
				requestBuilder[PrivateConstants.TX_IS_COMMITTED] = function(command) {
					...
					return [
						'F=' + PrivateConstants.TX_IS_COMMITTED + buildCommonParameters(command),
						'X=' + transaction.getTxID()
					];
				};
				...
				var buildRequest = function(command) {
					if (Utils.isLoggingEnabled()) {
						Utils.Logger.log(
							3,
							'[HTTP Request Factory] Building Request from Command ' + describe(command.requestType)
						);
					}
			...
			var HTTPResponseFactory = function(parentSession) {
				...
				responseHandlers[PrivateConstants.SESSION_START] = function(response, command) {
					if (Utils.isLoggingEnabled()) {
						Utils.Logger.log(
							6,
							'[HTTP Response Factory] Received Session Start Response for Command ' +
								command.requestType,
							response,
							command
						);
					...
					if (response[1]) {
						...
						parentSession.notifyListeners(CallbackConstants.ERROR, parentSession, {
							name: 'SessionConnectException',
							message: response[2]
						});
					}
				};
				...
				responseHandlers[PrivateConstants.BATCH_SUBSCRIBE] = function(response, command) {
					if (Utils.isLoggingEnabled()) {
						Utils.Logger.log(
							3,
							'[HTTP Response Factory] Received Batch Subscribe Callback for Command ' +
								command.requestType,
							response,
							command
						);
					...
					for (var i = 0; i < resourceCount; i++) {
						...
						if (success) {
							...
							if (!isResubscribe) {
								// Only notify on initial subscription
				...
				responseHandlers[PrivateConstants.RESOURCE_PUBLISH] = function(response, command) {
					if (Utils.isLoggingEnabled()) {
						Utils.Logger.log(
							3,
							'[HTTP Response Factory] Received Publish Callback for Command ' + command.requestType,
							response,
							command
						);
					}
				...
				responseHandlers[PrivateConstants.RESOURCE_UNSUBSCRIBE] = function(response, command) {
					if (Utils.isLoggingEnabled()) {
						Utils.Logger.log(
							3,
							'[HTTP Response Factory] Received Resource unsubscribe Callback for Command ' +
								command.requestType
						);
					}
					var resource = command.resource;
				...
				responseHandlers[PrivateConstants.PACKET_RECEIVE] = function(data) {
					if (parentSession.getLongPollRequestID() < data[1]) {
						...
						Transport.onError(
							'Data received out of order, reset required. Expected packet ' +
								(parentSession.getLongPollRequestID() + 1) +
								' but received ' +
								data[1]
						);
					}
					...
					if (!isDataGroup) {
						...
						if (!resource) {
							if (Utils.isLoggingEnabled()) {
								Utils.Logger.log(
									3,
									'[HTTP Response Factory] Event was for non-existent resource: ' + rawEvent.CNAME
								);
							}
							return;
						}
				...
				responseHandlers[PrivateConstants.QUEUE_COMMIT] = function(response, command) {
					if (Utils.isLoggingEnabled()) {
						Utils.Logger.log(
							3,
							'[HTTP Response Factory] Received Queue Commit Response',
							response,
							command
						);
					}
				...
				responseHandlers[PrivateConstants.QUEUE_ROLLBACK] = function(response, command) {
					if (Utils.isLoggingEnabled()) {
						Utils.Logger.log(
							3,
							'[HTTP Response Factory] Received Queue Rollback Response',
							response,
							command
						);
					}
				...
				responseHandlers[PrivateConstants.CLIENT_CLOSE] = function(response, command) {
					...
					if (command.clientRequest) {
						if (Utils.isLoggingEnabled()) {
							Utils.Logger.log(
								6,
								'[HTTP Response Factory] Received Client Close Session Response ' +
									describe(response[0]) +
									' for command ' +
									describe(command)
							);
						...
						if (Utils.isLoggingEnabled()) {
							Utils.Logger.log(
								4,
								'[HTTP Response Factory] Received Internal Close Session Response ' +
									describe(response[0]) +
									' for command ' +
									describe(command)
							);
				...
				function dispatchResponse(response) {
					...
					if (Utils.isLoggingEnabled()) {
						Utils.Logger.log(
							3,
							'[HTTP Response Factory] Dispatching response ' + describe(responseData[0])
						);
					}
					...
					if (
						messageID === PrivateConstants.EVENT_RECEIVE ||
						messageID === PrivateConstants.PACKET_RECEIVE ||
						messageID === PrivateConstants.SET_RESOURCE_ALIAS
					) {
			...
			nirvana_core.postMessageProxies = postMessageProxies;
			...
			function convertLongFromServer(longType, tmpValue) {
				if (tmpValue && Array.isArray(tmpValue)) {
					...
					if (longType == Nirvana.LongType.LONG_AS_DECIMAL_STRING) {
						...
						//Long as Number, we will loose precision past 53 bits
		...
		var Transport = (function() {
			...
			function setPreferredDriver() {
				...
				nirvana_core.preferredDrivers = [ preferredDriver ];
			...
			function useNextDriver() {
				if (Utils.isLoggingEnabled()) {
					Utils.Logger.log(4, '[Transport] Remaining drivers: ' + nirvana_core.preferredDrivers.join());
				}
				if (nirvana_core.preferredDrivers.length > 1) {
					currentDriverIndex = (currentDriverIndex + 1) % nirvana_core.preferredDrivers.length;
					if (currentDriverIndex > 0 || sessionConfig.realms.length === 1) {
						currentSession.notifyListeners(
							Nirvana.Observe.DRIVER_CHANGE,
							currentSession,
							nirvana_core.preferredDrivers[currentDriverIndex]
						);
					}
			...
			function useNextRealm() {
				...
				if (currentRealmIndex != newIndex) {
					...
					currentSession.notifyListeners(
						Nirvana.Observe.REALM_CHANGE,
						currentSession,
						sessionConfig.realms[currentRealmIndex]
					);
					currentDriverFailureCount = 0;
					nirvanaObj.setPreferredDrivers(
						currentSession.getConfig().drivers,
						sessionConfig.applyUserAgentHeuristics,
						false
					);
				}
			}
			...
			function initializeDriver() {
				if (nirvana_core.preferredDrivers.length) {
					var d = nirvanaObj.Drivers.createDriver(nirvana_core.preferredDrivers[currentDriverIndex]);
					if (Utils.isLoggingEnabled()) {
					...
					if (!d.isClientSupported()) {
						...
						currentSession.notifyListeners(
							Nirvana.Observe.DRIVER_CHANGE,
							currentSession,
							preferredDrivers[currentDriverIndex]
						);
			...
			function open() {
				...
				if (driver !== null) {
					...
					nirvanaObj.openListenerManager(currentSession, nvde);
				}
			}
			...
			function getCurrentDriver() {
				return nirvana_core.preferredDrivers[currentDriverIndex];
			}
				...
				if (currentSession.getStatus() === Statuses.STOPPED) {
					...
					if (currentDriverFailureCount >= MAX_DRIVER_FAILURES_BEFORE_CHANGE) {
						if (Utils.isLoggingEnabled()) {
							Utils.Logger.log(
								3,
								'[Transport] Driver failure reached maximum permitted count: ' +
									MAX_DRIVER_FAILURES_BEFORE_CHANGE
							);
						}
						useNextDriver();
					}
			...
			function scheduleReset(request, timeout) {
				...
				if (Utils.isLoggingEnabled()) {
					Utils.Logger.log(
						2,
						'[Transport] Setting Scheduler resetTimeoutID  : ' + resetTimeoutID + ' for request: ' + request
					);
				}
			}
			...
			function send(command) {
				...
				if (Utils.isLoggingEnabled()) {
					Utils.Logger.log(
						3,
						'[Transport] Sending request number ' +
							command.requestID +
							' of ' +
							requestCount +
							' for ' +
							describe(command.requestType) +
							' ' +
							request
					);
				}
			...
			function resetSession(reason) {
				...
				if (
					currentSession.getStatus() === Statuses.RECONNECTING ||
					currentSession.getStatus() === Statuses.CONNECTING
				) {
					realmRecoveryAttemptCount++;
					var queuedCommandCheck = OutboundEngine.peek();
					if (
						queuedCommandCheck === null ||
						queuedCommandCheck === undefined ||
						queuedCommandCheck.requestType !== PrivateConstants.SESSION_START
					) {
						OutboundEngine.prioritizeCommand({
							requestType: PrivateConstants.SESSION_START,
							session: currentSession,
					...
					if (Utils.isLoggingEnabled()) {
						Utils.Logger.log(
							6,
							'[Transport] Recreating Driver to ' +
			...
			//nirvana_core export
			nirvana_core.currentSession = currentSession;
			nirvana_core.preferredDrivers = preferredDrivers;
			nirvana_core.defaultDrivers = nirvanaObj.defaultDrivers;
			nirvana_core.PrivateConstants = PrivateConstants;
			nirvana_core.ListenerManagers = ListenerManagers;

			return {
				Drivers: nirvanaObj.Drivers,
		...
		//nirvana_core export Transport and Utils layer
		nirvana_core.Transport = Transport;
		nirvana_core.Utils = Utils;
		...
		var OutboundEngine = (function() {
			...
			function pushToTransport() {
				if (isReady && outboundQueue.length > 0) {
					...
					if (req !== null) {
						if (
							req.requestType == PrivateConstants.SESSION_START &&
							Transport.getCurrentSession().getStatus() === Statuses.CONNECTED
						) {
							Utils.Logger.log(
								3,
								'[Outbound Engine] Dequeueing unnecessary session start request as already connected.',
								'Req #' + req.requestID
							);
							...
							if (Utils.isLoggingEnabled()) {
								Utils.Logger.log(
									3,
									'[Outbound Engine] Sending Command to Transport Layer.',
									'Req #' + req.requestID
								);
							}
							Transport.send(req);
			...
			function prioritizeCommand(command) {
				...
				if (Utils.isLoggingEnabled()) {
					Utils.Logger.log(
						3,
						'[Outbound Engine] Prioritizing Command ' + describe(command.requestType),
						command
					);
				}
				pushToTransport();
			...
			function dequeueCommand(requestID) {
				...
				for (var i = 0; i < len; i++) {
					if (outboundQueue[i].requestID == requestID) {
						if (Utils.isLoggingEnabled()) {
							Utils.Logger.log(
								3,
								'[Outbound Engine] Dequeueing Command ' + describe(outboundQueue[i].requestType),
								outboundQueue[i],
								requestID
							);
						}
				...
				if (Utils.isLoggingEnabled()) {
					Utils.Logger.log(
						3,
						'[Outbound Engine] No matching queued command to dequeue for request ID: ',
						requestID
					);
				}
				return null;
			...
			function resetMessagesOnQueue() {
				...
				for (var i = 0; i < outboundQueue.length; i++) {
					if (
						outboundQueue[i].requestType === PrivateConstants.KEEP_ALIVE ||
						outboundQueue[i].requestType === PrivateConstants.CLIENT_CLOSE
					) {
		...
		//export OutboundEngine
		nirvana_core.OutboundEngine = OutboundEngine;
		...
		function createListenerManager() {
			...
			//nirvana_core export listeners
			nirvana_notifyListeners.notifyListeners = notifyListeners;
		...
		function AlreadySubscribedException(resourceName) {
			...
			if (typeof resourceName != 'undefined' && resourceName !== null)
				this.message = 'Already subscribed to ' + resourceName;
			if (Utils.isLoggingEnabled()) {
		...
		function InvalidDriverException(driverName) {
			...
			if (typeof driverName != 'undefined' && driverName !== null)
				this.message = driverName + ' is not a valid driver name';
			if (Utils.isLoggingEnabled()) {
				Utils.Logger.log(7, '[' + this.name + '] ' + this.message);
			}
		}
		...
		function DriverSecurityException(driverName, securityMessage) {
			...
			if (typeof driverName != 'undefined' && driverName !== null)
				this.message = driverName + ' - ' + securityMessage;
			if (Utils.isLoggingEnabled()) {
				Utils.Logger.log(7, '[' + this.name + '] ' + this.message);
			}
		}
		...
		var exceptions;
		exceptions = {
			InvalidDriverException: InvalidDriverException,
			SessionException: SessionException,
			DriverSecurityException: DriverSecurityException
		};
		//export exeptions
		nirvana_core.exceptions = exceptions;
		...
		function createSession(config) {
			if (CurrentSession) {
				if (
					CurrentSession.getStatus() !== Statuses.STOPPED &&
					CurrentSession.getStatus() !== Statuses.NOTSTARTED
				) {
			...
			function sanitizeConfiguration(config) {
				...
				if (typeof config.applicationName == 'undefined') config.applicationName = 'GenericJSClient';

				if (typeof config.sessionName == 'undefined') config.sessionName = 'GenericJSSession';

				if (typeof config.username == 'undefined') config.username = 'JSUser_' + Utils.generateUniqueID();

				if (typeof config.crossDomainPath == 'undefined') config.crossDomainPath = '/out';

				if (typeof config.realms == 'undefined') {
					config.realms = nirvanaObj.getFormatedUrlRealm(config);
				}

				var currentDomain = nirvanaObj.getCurrentDomainUrl();
				...
				if (Utils.isLoggingEnabled()) {
					...
					nirvanaObj.log(5, '[Session] Configuration:\n' + summary);
				}
        ...
        */
		...
		function on(observable, listener) {
			...
			if (lm === null) {
				registerListenerManager(Nirvana, lm);
			}
            ...
            */
			...
			this.start = function() {
				if (!(status === Statuses.NOTSTARTED || status === Statuses.STOPPED)) {
					var sessionEx = new SessionException(
						'Nirvana Session',
						'start() can only be invoked on a new or stopped session'
					);

					nirvanaObj.notifyListenersManagerStart(client, sessionEx);
					return client;
				}

				var usePreviousBestDriverFlag = status === Statuses.STOPPED;

				if (nirvanaObj.getElementofDriverDomOnjContainer() === null) {
					Nirvana.initialize();
					if (Utils.isLoggingEnabled()) {
				...
				nirvanaObj.addClientEventListener(bu);
				return client;
			};
				...
				try {
					...
					for (var i = 0; i < resourceArray.length; i++) {
						...
						if (clientResource.isSubscribed()) {
							...
							nirvanaObj.subscribeListenerManager(client, ase);
						} else {
					...
					nirvanaObj.subscribeNotifyListener(client, ex);
				}
				return client;
			};
        ...
        */
		...
		EventAttributes.SERVER_HEADER_KEYS = [
			'timeStamp',
			'_isDelta',
			'_isRedelivered',
			'_isRegistered',
		...
		EventAttributes.JMS_HEADER_KEYS = [
			'messageID',
			'correlationID',
			'type',
			'expiration',
        ...
        */
		...
		return {
			...
			BUILD_NUMBER: 'BuildNumberHere',
			BUILD_DATE: '12-Feb-1964',
			...
			Driver: nirvanaObj.Drivers.Names,
			clientSupportsDriver: Transport.supportsDriver,

			// Framework Level Methods
			initialize: nirvanaObj.initialize,
			createSession: createSession,
			createEvent: createEvent,
			createDictionary: createDictionary,
			on: on,
			removeListener: removeListener,
			nirvana_core: nirvana_core,
			nirvanaObj: nirvanaObj,
			/*
                Class: Nirvana.Utils
	...
	/**export Nirvana and nirvana_core */
	if (!isBrowser) {
		module.exports.Nirvana = Nirvana;
		module.exports.nirvana_core = Nirvana.nirvana_core;
	}

	/**This will export nirvana_core obj from current file
	 * to nirvana_browser.js and will remove the object
	 */
	function implNirvanaCoreObj() {
		Nirvana.nirvanaObj.implNirvanaCore(Nirvana.nirvana_core);
		if (typeof Nirvana === undefined || typeof Nirvana == 'undefined') {
		} else {
			Nirvana.nirvana_core = {};
			Nirvana.nirvanaObj = {};
			delete Nirvana.nirvana_core;
			delete Nirvana.nirvanaObj;
		}
	}
...
/**impl Nirvana Object
 * it will give us the nirvana_browser.js functions
 * this object is hidden from customers
 */
function implnirvanaObjTemp(nirvanaObjTempVariable) {
	nirvanaObjJS = nirvanaObjTempVariable;
}
