...
class DLL fSSLConfig :
    public fSocketConfig, public fBaseSSLConfig {
  public:
    fSSLConfig(void);
    fSSLConfig(const std::string& name, const std::string& adapter, int port, bool autostart = true);
    virtual ~fSSLConfig(void);
  ...
  protected:
    bool m_bCertRequired;
    std::string m_alias;
    std::string m_privatePassword;
    std::string *m_ppCiphers;
    int m_nCipher;
