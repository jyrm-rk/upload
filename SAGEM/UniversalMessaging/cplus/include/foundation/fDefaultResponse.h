/*
 *
 *   Copyright (c) 1999 - 2011 my-Channels Ltd
 *   Copyright (c) 2012 - 2020 Software AG, Darmstadt, Germany and/or Software AG USA Inc., Reston, VA, USA, and/or its subsidiaries and/or its affiliates and/or their licensors.
 *
 *   Use, reproduction, transfer, publication or disclosure is prohibited except as specifically provided for in your License Agreement with Software AG.
 *
 */
#pragma once

#include "fBaseTransportObject.h"
#include "fLoginResponse.h"

namespace com {
namespace pcbsys {
namespace foundation {
namespace io {
class fEventInputStream;
}

namespace security {

using namespace fbase;
using namespace io;

class fDefaultResponse :
    public fLoginResponse, fBaseTransportObject {
  public:
    fDefaultResponse(bool);
    fDefaultResponse(void);
    virtual ~fDefaultResponse(void);

    virtual void readExternal(fEventInputStream *pIstream);
    virtual void writeExternal(fEventOutputStream *pOstream);

  private:
    bool m_bResponse;
};

}
}
}
}
